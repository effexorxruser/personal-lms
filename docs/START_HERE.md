# START HERE

Если вы впервые в проекте, идите в таком порядке.

## 1. Общее состояние репозитория

- [README.md](../README.md)
- Локальные операции с пользователями: `scripts/reset_users.py` (описано в README, раздел "Быстрый старт")

## 2. Продуктовый смысл

- [PRODUCT_VISION.md](product/PRODUCT_VISION.md)

## 3. Границы MVP

- [MVP_SCOPE.md](product/MVP_SCOPE.md)

## 4. Как устроено обучение

- **Контракт course pack и авторинг файлов:** [course_authoring/COURSE_PACK_CONTRACT.md](course_authoring/COURSE_PACK_CONTRACT.md) · [COURSE_AUTHORING_GUIDE.md](course_authoring/COURSE_AUTHORING_GUIDE.md)
- [LEARNING_MODEL.md](product/LEARNING_MODEL.md)
- [CONTENT_STRATEGY.md](product/CONTENT_STRATEGY.md)
- [SOURCE_STACK.md](product/SOURCE_STACK.md)
- [AUTHORING_MODEL.md](product/AUTHORING_MODEL.md)
- [CURRICULUM_ROADMAP.md](product/CURRICULUM_ROADMAP.md)
- [BACKEND_DEVELOPER_COMPETENCY_MAP.md](product/BACKEND_DEVELOPER_COMPETENCY_MAP.md)
- [CURRICULUM_DECISION_RECORD.md](product/CURRICULUM_DECISION_RECORD.md)

## 5. Архитектура и runtime

- **Целевая full-stack архитектура (Next.js 15):** [architecture/NEXT_FULLSTACK_ADAPTATION.md](architecture/NEXT_FULLSTACK_ADAPTATION.md)
- **Course Knowledge Graph:** [architecture/COURSE_KNOWLEDGE_GRAPH.md](architecture/COURSE_KNOWLEDGE_GRAPH.md)
- **Текущая runtime (FastAPI, previous):** [ARCHITECTURE_OVERVIEW.md](product/ARCHITECTURE_OVERVIEW.md)

## 6. Ограничения

- [PROJECT_GUARDRAILS.md](product/PROJECT_GUARDRAILS.md)

Этот порядок важен: сначала текущее состояние кода, потом продуктовые ограничения и архитектура.
