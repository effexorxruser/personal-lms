...
## AI helper architecture (Lain v0.5)
...
- API: `POST /api/ai/helper`, `GET /api/ai/helper/history`;
- service: `app/services/ai_helper_service.py`;
- provider adapter: `app/services/lain_provider.py`;
- runtime log: `LainHelperInteraction`;
- modes типизированы через `LainHelperMode` (единый source of truth);
- guardrails на уровне prompt + scope/refusal logic;
- режимы: `explain_lesson`, `help_start`, `stuck_help`, `submission_hint`, `free_question`;
- mini-chat подгружает recent history текущего урока при первом открытии панели.
