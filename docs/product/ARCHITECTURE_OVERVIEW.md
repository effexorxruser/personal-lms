...
Terminal sandbox/whitelist в MVP не является полноценной security boundary для untrusted/public execution.

## AI helper architecture (Lain v1.0)

Lain встроена как context-scoped tutor channel, а не как отдельный generic chatbot.

- UI: floating launcher/capsule + mini-chat panel в базовом layout;
- API: `POST /api/ai-helper/history`, `POST /api/ai-helper/chat`, `POST /api/ai-helper/clear`;
- service: `app/services/ai_helper_service.py`;
- runtime log: `AIHelperMessage`;
- history model: отдельные диалоги по `context_key` (lesson/course/page);
- guardrails: русский язык, краткие ответы, off-topic refusal + возврат к текущему учебному шагу;
- panel поддерживает online/offline/thinking state и отдельный сократический toggle;
- mini-chat подгружает историю текущего контекста при открытии.
