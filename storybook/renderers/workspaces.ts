...
export function renderDashboardWorkspace(args: { checkpointState: UiState; blockerActive: boolean; density: 'compact' | 'normal' }): HTMLElement {
  const content = `
    ...
    ${renderSectionNav({ items: ['Центр', 'Сводка', 'Модули'] })}
    <div class="dashboard-layout section-flow">
      ...
      <aside class="dashboard-rail section-block">
...
export function renderCourseMapWorkspace(args: { expanded: boolean; checkpointState: UiState }): HTMLElement {
  const content = `
    ...
    <section class="module-list">${renderModuleAccordionItem({ title: '1. Модуль 1: Основа backend', description: 'Базовые принципы backend-приложения.', state: args.checkpointState, expanded: args.expanded, checkpointState: args.checkpointState })}</section>
  `;
...
export function renderLessonWorkspace(args: { state: UiState; reviewApproved: boolean }): HTMLElement {
  const content = `
    ...
    <section class="lesson-workspace"><article class="markdown-body lesson-main"><h1>Структура backend</h1><p>Текущий каркас использует <strong>FastAPI</strong>, шаблоны Jinja2 и файловый контент.</p><p>Ключевой принцип: данные курса живут только в content/.</p></article>${renderExecutionRail({ taskTitle: 'Проверить структуру приложения', state: args.state, reviewApproved: args.reviewApproved })}</section>
  `;
...
export function renderRecapWorkspace(args: { blockers: number; reviews: number }): HTMLElement {
  const content = `
    ...
    ${renderSectionNav({ items: ['Обзор', 'Уроки', 'Практика', 'Фокус'] })}
    ${renderRecapSummary({ lessons: 3, submissions: 4, reviews: args.reviews, blockers: args.blockers })}
    <section class="recap-grid section-flow"><article class="ui-card recap-card"><p class="status-kicker">Завершённые уроки</p><ul><li><strong>Урок 1: Введение в трек</strong><span>завершено 17.04</span></li><li><strong>Урок 2: Структура backend</strong><span>завершено 17.04</span></li></ul></article><article class="ui-card recap-card recap-card--focus"><p class="status-kicker">Следующий фокус</p><h2>Foundation artifact</h2><a class="btn btn--primary" href="#">Продолжить</a></article></section>
  `;
